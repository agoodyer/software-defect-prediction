...
Research in the field of Software Defect Prediction is typically focused on maximizing the predictive performance of models through various machine learning approaches to the problem. A foundational study by Menzies et al. (2007) pioneered the use of the NASA Promise dataset, and argued that static code metrics are sufficient to build effective defect prediction models. 

While static metrics are widely used for defect prediction, later works have scrutinized the use of these metrics for defect prediction. Shepperd et al. (2013) criticizes the Promise dataset due to several flaws including data duplication, redundant features, and inconsistencies that hinder the validity of results. This led to the creation of cleaned versions of the dataset like $D''$ that address many of the data quality issues identified with Promise. 

Our researched is positioned to shift focus towards model interpretability and simplicity in our methods rather than predictive performance. While 
